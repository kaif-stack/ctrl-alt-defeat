...
const Navbar = () => {
  ...
  return (
    <section id="navbar" className="bg-yellow-500 z-50 shadow-md w-full fixed top-0 left-0">
      <div className="md:flex items-center justify-between bg-theme py-4 xl:px-16 px-6 md:px-14">
        ...
        >
          ...
          <button onClick={() => setJoinOpen(!joinOpen)}
            className="bg-theme md:text-[9px]  xl:text-[15px] font-normal tracking-wider leading-5 text-[#F9F7F7] hover:text-theme border-[#F9F7F7] border-2 font-defaultFont px-5 py-1 rounded md:ml-8 hover:bg-red-700
    duration-500"
          >
            Join
          </button>
          ...
          {joinOpen && (
            <div className="w-[110px] absolute right-12 z-10 py-2 bg-[#f9f7f7]  text-theme rounded-lg mt-2">
              <Link to="/registerBusiness" className="block px-2 py-1   hover:bg-[#dbe2ef]">
                As Business
              </Link>
              <Link to="/register" className="block px-2 py-1 hover:bg-[#dbe2ef]">
                 As User
              </Link>
            </div>
          )}
